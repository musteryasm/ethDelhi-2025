...
import ABI from './frontend/contractABI.json' assert { type: 'json' };
...
async function testIntegration() {
  ...
  try {
    ...
    console.log(`✅ Contract balance: ${ethers.formatEther(balance)} CT`);
    ...
    if (contestCountNum > 0) {
      for (let i = 0; i < Math.min(contestCountNum, 3); i++) {
        try {
          ...
          console.log(`✅ Contest ${i}: ${contest[0]} (Stake: ${ethers.formatEther(contest[1])} CT)`);
        } catch (error) {
