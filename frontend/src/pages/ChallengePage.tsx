  ...
  const joinContest = async (contest) => {
    ...
    try {
      ...
      if (balance < requiredAmount) {
        alert(`Insufficient balance. You have ${ethers.formatEther(balance)} ETH but need ${ethers.formatEther(requiredAmount)} ETH`);
        return;
      }
      ...
      try {
        ...
        const estimatedGas = await contract.joinContest.estimateGas(contest.id, {
        ...
        try {
          const contractRead = new ethers.Contract(CONTRACT_ADDRESS, [
            ...CONTRACT_ABI,
          ...
          if (!contestData || contestData[0] === '') {
            alert('Contest does not exist on the blockchain. Please check if it was created properly.');
            return;
          }
          
        } catch (readError) {
          console.error('Failed to read contract:', readError);
          alert('Contract interaction failed. Please check if you are on the correct network (Sepolia).');
          return;
        }
      ...
      if (error.message.includes('user rejected')) {
        ...
        errorMessage += 'Contract call failed. Check if contest exists and you meet all requirements.';
      } else {
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          <div className="text-sm text-gray-400">
            Total Contests: <span className="text-cyber-green font-bold">{contests.length}</span>
            <span className="ml-2 text-xs">
              {contests.length > 0 && contests[0].name?.includes('Fallback') ? '(Using Fallback Data)' : '(Live from API)'}
            </span>
          </div>
          {account ? (
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {contests.map((contest) => {
          ...
          const isActive = parseInt(contest.startTime) <= Date.now() / 1000 && 
              ...
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <div className="flex items-center space-x-4 text-sm text-gray-400">
                    {contest.difficulty && (
                      <span className={`px-2 py-1 rounded text-xs ${getDifficultyColor(contest.difficulty)}`}>
                        {contest.difficulty}
                      </span>
                    )}
                  ...
                  <div className="mt-2">
                    <span className={`px-2 py-1 rounded text-xs ${
                      isActive ? 'bg-green-900 text-green-300' : 'bg-gray-900 text-gray-300'
                    }`}>
      ...
      {!account && (
        <GlassCard className="text-center py-8">
          ...
          <p className="text-gray-400 mb-4">
            Connect your MetaMask wallet to join contests and start earning rewards
          </p>
      ...
      {/* Camera Modal (unchanged from your original) */}
      {showCameraModal && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
              ...
              <p className="text-gray-300">
                {selectedChallenge ? (
                  <>
                    ...
                    <br />
                    Stake: <span className="font-bold">{formatStake(selectedChallenge.stakeAmount)}</span>
                  </>
                ) : (
